...
<section class="w-full h-auto bg-jhc-2 text-white py-28 relative overflow-hidden" id="kontak">
  ...
  <div class="container max-w-6xl mx-auto flex flex-col gap-6 justify-center items-center relative z-10">
    ...
    <div class="flex flex-col gap-1 w-6/12 mx-auto px-auto py-6">
      {#each contacts as con}
        <a
          href={con.link}
          target="_blank"
          class="flex flex-row gap-2 items-center hover:underline hover:text-jhc-light h-12"
        >
          <img
            src={con.img}
            alt={con.title}
